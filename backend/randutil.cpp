...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
std::string RAND_UTIL::getComputerName( void ) const
{
#ifdef __BORLANDC__
	return( "UNDETERMINED" );
#else
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool RAND_UTIL::isLive( void ) const
{
	return( is_live );
}
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
void RAND_UTIL::htmlBeginDoc( const std::string title )
{
	...
	std::vector<std::string>::const_iterator    hi = head_include.begin();
	while ( hi != head_include.end() )
		{printf( "\n%s", hi->c_str() );
		hi++;
		}
	const   std::string     caller_ip = XCGI::makeWebSafe(
		determineCallerIP() );
	printf( "\n<!-- Caller IP %s -->\n</head>\n<body>", caller_ip.c_str() );
}
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void RAND_UTIL::addHeadInclude( const std::string inc_link )
{
	head_include.push_back( inc_link );
}
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void RAND_UTIL::addHeadIncludeStyle( const std::string sty )
{
	const	std::string s = std::string( "<link rel=\"stylesheet\" href=\"")
		+ sty + "\" type=\"text/css\">";
	addHeadInclude( s );
}
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
{
	addHeadIncludeStyle( "rand.css" );
	htmlBeginDoc( title );
}
//---------------------------------------------------------------------------
void RAND_UTIL::htmlBeginBodyFormat( void )
{
...
"<img src=\"logo_oxuniv_square.png\" alt=\"Oxford University Logo\"></a>"
	"\n</div>"
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void RAND_UTIL::htmlEndBodyFormat( void )
{
	printf( "\n</div>" );
}
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
