...
void ESG::init( void )
{
	...
	if ( NULL == row || NULL == col || NULL == cols )
		{is_valid = false;
		return;
		}
	cols[0] = NULL;
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void ESG::clear( void )		// RESET TO INITIAL
{
	remove();
	init();
}
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool ESG::isValid( void ) const
{
	return( is_valid );
}
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
//---------------------------------------------------------------------------
				/* BLEND COLOURS ACCORDING TO WEIGHTS */
TColor ESG::colourBlend( const TColor ca, int wa, const TColor cb, int wb )
{
	int     i;
	int     c = 0;
	int     pa[3], pb[3];
	if ( wa < 0 || wa > 999999 )
		{wa = 0;           	// PREVENT [OVER|UNDER]FLOW ERRORS
		}
	if ( wb < 0 || wb > 999999 )
		{wb = 0;
		}
	if ( 0 == wa && 0 == wb )
		{			// BLEND EVENLY IF PARAMETERS BAD
		wa = 1;
		wb = 1;
		}
	int	ica = (int) ca;
	...
	for ( i = 2; i >= 0; i-- )
		{c *= 256;              // BLEND COLOUR COMPONENTS
		c += ( ( wa * pa[i] ) + ( wb * pb[i] ) ) / ( wa + wb );
		}
	return( (TColor) c );
}
//---------------------------------------------------------------------------
