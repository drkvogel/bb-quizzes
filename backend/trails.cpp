...
void Trails::parseResponses(TrailsRecord *rec) { //IFDEBUG printf("<code>parseResponses():");
    try {
...
Trails::TrailsRecord Trails::getPayload(XCGI * x) { // get responses from frontend
    IFDEBUG printf("<p><code>this is getPayload() in %s.<br />\n", __FILE__); //int np = x->param.count();
    TrailsRecord rec;
    ...
    IFDEBUG printf(", tinstruct.iso(): '%s', tstart.iso(): '%s', tfinish.iso(): '%s'<br />",
...
bool Trails::insertRecord(const TrailsRecord *rec) {
    IFDEBUG printf("<p><code>Trails::insertRecord()\n");
...
void Trails::printRecords() {
    ...
    for (int i = 1; i <= NUM_POINTS_TOTAL; i++) {
        ...
        printf("<td>%s_%d_wrong</td><td>%s_%d_time</td><td>%s_%d_total</td>", sect, clik, sect, clik, sect, clik);
    }
    printf("</thead>\n");
