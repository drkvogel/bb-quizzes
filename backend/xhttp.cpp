...
int xhttp_send_request(char **response, char *request) {
    ...
    }; //DBG_PRINT("-sent"); //DBG_PRINT2(" req:\n%s\n", obuf);

    // Loop on receive (receive flags a dropped connection by returning zero)
    ilen = 0;
    iret = recv(sock, ibuf, sizeof(ibuf), 0); //DBG_PRINT("-received");
    ...
    *response = (char *)malloc((iret + 1) * sizeof(char));
    while (iret > 0) {
        *response = (char *)realloc((void *)(*response), (ilen + iret + 1)*sizeof(char));
        iptr = *response;
        iptr += ilen;
        strncpy(iptr, ibuf, iret); // results becomes corrupted after this?
        ilen += iret;
...
int xhttp_post(char **response, char *url, char *content, char *contentType, char *host) {
    ...
    char *requestbuf;
    char *contentbuf;
    char * header = "POST %s HTTP/1.0\rContent-Type: %s\rHost: %s\rContent-Length: %d\n\n%s";
/*    requestbuf = (char *)malloc((strlen("POST ") + strlen(url) + strlen (" HTTP/1.0\n\nContent-Type: \r")
        + strlen(contentType) + strlen("Host: \r") + strlen(host) + strlen("Content-Length: \n\n") + strlen(content) + 20) * sizeof(char));*/
    requestbuf = (char *)malloc((strlen(header) + strlen(url) + strlen(contentType) + strlen(host) + strlen(content) + 20) * sizeof(char));
    sprintf(requestbuf, header, url, contentType, host, strlen(content), content);
    result = xhttp_send_request(response, requestbuf);
    free(requestbuf);
    return result;
}
