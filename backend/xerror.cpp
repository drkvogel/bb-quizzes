...
#endif
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
#define	outputErrnoDef(c )		if ( n == c ) {return( #c ); }
std::string XERROR::translateErrno( const int n )
{
	outputErrnoDef( EACCES );
	outputErrnoDef( EEXIST );
	outputErrnoDef( EIO );
	outputErrnoDef( EMLINK );
	outputErrnoDef( ENAMETOOLONG );
	outputErrnoDef( ENOENT );
	outputErrnoDef( ENOSPC );
	outputErrnoDef( ENOTDIR );
	outputErrnoDef( EROFS );
#ifndef __BORLANDC__
	outputErrnoDef( ELOOP );
	outputErrnoDef( EOVERFLOW );
#endif
	return( "UNKNOWN" );
}
#undef outputErrnoDef
//===========================================================================
