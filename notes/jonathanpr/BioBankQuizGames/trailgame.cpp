...
//===========================================================================
TRAIL_GAME::TRAIL_GAME( TImage *board, const TColor background,	void (* showMatchFn)(),	void (* resetBoardFn)(), void (* resetUserTimeOutFunction)(bool),TButton *NextButton )
	:flashBGFunction( showMatchFn ),resetBGFunction(resetBoardFn),resetUserTimeOut(resetUserTimeOutFunction),left_start( 15 ),left_gap( 30 ),top_gap( 30 ),top_start( 10 ), TheTrail( board, background )
{
...
void TRAIL_GAME::NextClicked()
{
	...
	if (m_GameID == -1)
		m_returnDataHeader.introelapsed = milliSecondsToDeciSeconds(timeNow,m_gameTime);  //milliseconds elapsed since 1 January 1970
	else if (m_GameID == 0)
		m_returnDataHeader.practiceaelapsed = milliSecondsToDeciSeconds(timeNow,m_gameTime);  //milliseconds elapsed since 1 January 1970
	else if (m_GameID == 1)
		m_returnDataHeader.partaelapsed = milliSecondsToDeciSeconds(timeNow,m_gameTime);  //milliseconds elapsed since 1 January 1970
	else if (m_GameID == 2)
		m_returnDataHeader.practicebelapsed = milliSecondsToDeciSeconds(timeNow,m_gameTime);  //milliseconds elapsed since 1 January 1970
	else if (m_GameID == 3)
		m_returnDataHeader.partbelapsed = milliSecondsToDeciSeconds(timeNow,m_gameTime);  //milliseconds elapsed since 1 January 1970
...
void TRAIL_GAME::drawText(std::vector<String> &info, bool showButton, int Size, System::Uitypes::TColor TextColour)
{
	...
	unsigned int i=0;
	for (;i<info.size();i++)
		m_playing_board->Canvas->TextOut(m_PosMinX, m_PosMaxY+25+25*i, info[i]);

	if (showButton)
	{
		m_NextButton->Left = m_PosMinX;
		m_NextButton->Top = m_PosMaxY+25+25+25*i;
		m_NextButton->Show();
	}
	else
		m_NextButton->Hide();
...
void TRAIL_GAME::NewSaveRow()
{
	...
	System::TDateTime timeNow = Now();
	m_CurrentRow->elapsed = milliSecondsToDeciSeconds(timeNow,m_trailClickTime);
}

// ---------------------------------------------------------------------------

long TRAIL_GAME::milliSecondsToDeciSeconds(System::TDateTime &timeNow,System::TDateTime &timeThen)
{
...
void TRAIL_GAME::userTimedOut()
{
   m_returnDataHeader.abandon= 3;
   is_running = false;
}
...
//live b coordinates
	...
	m_praca_inst.push_back(L"We now want to measure your co-ordination");
	m_praca_inst.push_back(L"");
	...
	m_pracb_inst.push_back(L"This is a practice. Beginning with number 1 join all the circles");
	m_pracb_inst.push_back(L"alternating between numbers and letters in ascending order, e.g. 1 A");
	m_pracb_inst.push_back(L"2 B. Please work as quickly and accurately as you can.");
