...
private:
	...
	void    (* resetUserTimeOut)(bool);
	void drawText(std::vector<String> &info, bool showButton, int Size = 15, System::Uitypes::TColor TextColour = clBlack);
	...
	void wipeTrail();
public:
	TRAIL_GAME( TImage *board, const TColor background,void (* flashBGFunction)(), void (* resetBGFunction)(), void (* resetUserTimeOutFunction)(bool),TButton *NextButton);
	~TRAIL_GAME( void );
	...
	void NextClicked();
	void userTimedOut(); //user not pressed anything for 30 seconds
	TRect getGameRec();
